...
class QuotationController extends Controller {
    ...
    public function export(){
        ...
        return response()->stream(function() use ($rows){ $out=fopen('php://output','w'); $hideCommercial=config('emko.hide_commercial_values'); fputcsv($out,['Tanggal','Status','Nama','Perusahaan','Email','Telepon','Produk','Qty','Lokasi','Kebutuhan Teknis']); foreach($rows as $r){ fputcsv($out,[$r->created_at,$r->status,$r->lead->name,$r->lead->company,$r->lead->email,$r->lead->phone,$r->product->product_name,$hideCommercial ? '' : $r->quantity,$r->project_location,$r->technical_needs]); } fclose($out); }, 200, ['Content-Type'=>'text/csv','Content-Disposition'=>'attachment; filename="emko-rfq.csv"']);
    }
}
