...
<section class="detail-hero product-detail-hero">
    ...
    <aside class="quote-box"><span>Harga Estimasi</span><strong>{{ $product->formatted_final_price_idr }}</strong><p>Harga awal {{ $product->formatted_price_idr }} | Diskon {{ number_format($product->discount_percent,0) }}%</p><small>{{ $product->price_note }}</small></aside>
</section>
