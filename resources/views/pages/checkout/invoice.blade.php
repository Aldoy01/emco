...
<section class="section invoice-layout">
    <div class="invoice-paper">
        ...
        <div class="table-wrap"><table><thead><tr><th>Produk</th><th>Qty</th><th>Harga Unit</th><th>Subtotal</th></tr></thead><tbody><tr><td>{{ $order->product->product_name }}</td><td>{{ $order->quantity }}</td><td>{{ $order->formatted_unit_price }}</td><td>{{ $order->formatted_subtotal }}</td></tr><tr><td colspan="3">Subtotal harga dasar</td><td>{{ $order->formatted_original_subtotal }}</td></tr><tr><td colspan="3">Diskon produk</td><td>- {{ $order->formatted_discount }}</td></tr><tr><td colspan="3">Subtotal setelah diskon</td><td>{{ $order->formatted_subtotal }}</td></tr><tr><td colspan="3">{{ $order->tax_label }}</td><td>{{ $order->formatted_tax }}</td></tr><tr><td colspan="3">Biaya pengiriman</td><td>{{ $order->formatted_shipping_cost }}</td></tr><tr><td colspan="3"><strong>Total Invoice</strong></td><td><strong>{{ $order->formatted_total }}</strong></td></tr></tbody></table></div>
