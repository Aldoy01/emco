...
class DatabaseSeeder extends Seeder {
    public function run(){
        ...
        foreach($products as $i=>$p){ [$cat,$name,$price,$final,$desc]=$p; Product::updateOrCreate(['slug'=>Str::slug($name)], ['category_id'=>$catModels[$cat]->id,'product_code'=>$name,'product_name'=>$name,'short_description'=>$desc,'features'=>['Harga estimasi Rupiah setelah diskon 5%','Cocok untuk kebutuhan proyek B2B','Dapat dikonsultasikan dengan sales sebelum pembelian'],'specifications'=>['Kategori: '.$cat,'Status: Active','Delivery dihitung berdasarkan alamat customer'],'price_usd'=>$price,'discount_percent'=>5,'final_price_usd'=>$final,'price_note'=>'Belum termasuk pajak, shipping, instalasi, konfigurasi, dan biaya lain berdasarkan proyek.','status'=>'active','is_featured'=>$i<6]); }
    }
}
